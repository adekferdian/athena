...
;(function () {
  ...
  var ReceivingSchema = [
    requiredOf(
      ...
      setOf(
        Result.value,
        objOfOnly([onStrictHandler('onReceive'), defaulted('schema', anyValue())])
      )
    ),
  ]
  ...
  var record = function (spec) {
    var uid =
      isSketchSpec$1(spec) && hasNonNullableKey(spec, 'uid') ? spec.uid : generate$2('memento')
  ...
  var generateFrom$1 = function (spec, all) {
    ...
    var validated = asRaw('component.behaviours', objOf(schema), spec.behaviours).fold(function (
      errInfo
    ) {
  ...
  var combineGroups = function (byEventName, eventOrder) {
    var r = mapToArray(byEventName, function (tuples, eventName) {
      var combined =
        tuples.length === 1 ? Result.value(tuples[0].handler) : fuse(tuples, eventOrder, eventName)
  ...
  var make$1 = function (detail, _rawUiSpec) {
    ...
    var keyOnItem = function (f) {
      return function (container, simulatedEvent) {
        return closest$1(simulatedEvent.getSource(), '.' + detail.markers.item).bind(function (
          target
        ) {
          return container
            .getSystem()
    ...
    requiredOf(
      ...
      setOf(
        ...
        setOf(
          Result.value,
          objOfOnly([
            optionObjOfOnly('transition', [required$1('property'), required$1('transitionClass')]),
          ])
        )
      )
    ),
  ...
  var create = function (platform, mask) {
    ...
    var enter = function () {
      ...
      getActiveApi(platform.editor).each(function (editorApi) {
        ...
        iosEvents.set(
          initEvents(editorApi, iosApi, platform.toolstrip, platform.socket, platform.dropup)
        )
      })
    }
    var exit = function () {
