...
;(function () {
  ...
  var get = function (patternsState) {
    ...
    var getPatterns = function () {
      return __spreadArray(
        __spreadArray([], map(patternsState.get().inlinePatterns, denormalizePattern), true),
  ...
  var textBefore = function (node, offset, rootNode) {
    if (isText(node) && offset >= 0) {
      ...
      return Optional.from(textSeeker.backwards(node, offset, alwaysNext(node), rootNode)).map(
        function (prev) {
  ...
  var textAfter = function (node, offset, rootNode) {
    if (isText(node) && offset >= node.length) {
      ...
      var textSeeker = global(DOM)
      return Optional.from(textSeeker.forwards(node, offset, alwaysNext(node), rootNode)).map(
        function (prev) {
  ...
  var scanLeft = function (node, offset, rootNode) {
    ...
    if (offset >= 0 && offset <= text.length) {
      ...
      var textSeeker = global(DOM)
      return Optional.from(textSeeker.backwards(node, offset, alwaysNext(node), rootNode)).bind(
        function (prev) {
  ...
  var scanRight = function (node, offset, rootNode) {
    ...
    if (offset <= text.length) {
      ...
      var textSeeker = global(DOM)
      return Optional.from(textSeeker.forwards(node, offset, alwaysNext(node), rootNode)).bind(
        function (next) {
