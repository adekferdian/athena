...
import DeleteVoucherRulesModal from '../components/DeleteVoucherRulesModal'
import {deleteVoucherRules, getVoucherRulesList} from '../redux/VoucherRulesCRUD'
import {getErrorMessage} from 'src/app/utils/api-utils'
import Pagination from 'src/app/components/Pagination'

const ListVoucherRules: FC = () => {
  ...
  const {state, setLimit, setPage, setQuery} = usePagination<VoucherRules, any>(
    useCallback((state, setState, isActive) => {
      const fetchCall = async () => {
        try {
          ...
          const masterBenefitType = await getVoucherRulesList({
            limit: state.limit,
  ...
  useEffect(() => {
    document.title = getTitle('List of Voucher Rules')
  }, [])
  ...
  return (
    <>
      <PageTitle>Manage Voucher Rules</PageTitle>
      <div className='card shadow-sm'>
        <div className='card-header d-flex align-items-center justify-content-between'>
          <div className='flex-fill fs-2 fw-bolder'>
            {VoucherRulesScreens.LIST_VOUCHER_RULES.TITLE}
          </div>
          <div className='card-toolbar'>
            <Link
              to={{
                pathname: `/voucher-rules/add`,
              }}
                  ...
                  {state.data.length === 0 ? (
                    ...
                    state.data.map((value, index) => (
                      <tr key={index}>
                        ...
                        <td className='align-middle'>{value.voucher.voucher_name}</td>
                        <td className='align-middle'>{value.rules.rule_name}</td>
                        <td className='align-middle'>{value.component.component_name}</td>
                        <td className='align-middle' style={{minWidth: 125}}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              history.push(`/voucher-rules/edit/${value.id}`)
                            }
                          >
        ...
        <DeleteVoucherRulesModal
          onDelete={() => {
            deleteVoucherRules(handleDeleteData?.id ?? '')
              .then(() => {
