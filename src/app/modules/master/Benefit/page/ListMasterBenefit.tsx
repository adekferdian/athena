...
import DeleteMasterBenefitModal from '../components/DeleteMasterBenefitModal'
import {deleteMasterBenefit, getMasterBenefitList} from '../redux/MasterBenefitCRUD'
import {getErrorMessage} from 'src/app/utils/api-utils'
import Pagination from 'src/app/components/Pagination'

const ListMasterBenefit: FC = () => {
  ...
  const {state, setLimit, setPage, setQuery} = usePagination<MasterBenefit, any>(
    useCallback((state, setState, isActive) => {
      const fetchCall = async () => {
        try {
          ...
          const masterBenefitType = await getMasterBenefitList({
            limit: state.limit,
  ...
  useEffect(() => {
    document.title = getTitle('List of Master Benefit')
  }, [])
  ...
  return (
    <>
      <PageTitle>Manage Master Benefit</PageTitle>
      <div className='card shadow-sm'>
        <div className='card-header d-flex align-items-center justify-content-between'>
          <div className='flex-fill fs-2 fw-bolder'>
            {MasterBenefitScreens.LIST_MASTER_BENEFIT.TITLE}
          </div>
          <div className='card-toolbar'>
            <Link
              to={{
                pathname: `/master-benefit/add`,
              }}
                  ...
                  {state.data.length === 0 ? (
                    ...
                    state.data.map((value, index) => (
                      <tr key={index}>
                        ...
                        <td className='align-middle'>{value.benefit_id}</td>
                        <td className='align-middle'>{value.benefit_type}</td>
                        <td className='align-middle'>{value.benefit_name}</td>
                        <td className='align-middle'>{value.benefit_description}</td>
                        <td className='align-middle'>{value.meta_data}</td>
                        ...
                        <td className='align-middle' style={{minWidth: 125}}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              history.push(`/master-benefit/edit/${value.benefit_id}`)
                            }
                          >
        ...
        <DeleteMasterBenefitModal
          onDelete={() => {
            deleteMasterBenefit(handleDeleteData?.benefit_id ?? '')
              .then(() => {
