...
import DeleteMasterComponentModal from '../components/DeleteMasterComponentModal'
import {deleteMasterComponent, getMasterComponentList} from '../redux/MasterComponentCRUD'
import {getErrorMessage} from 'src/app/utils/api-utils'
import Pagination from 'src/app/components/Pagination'

const ListMasterComponent: FC = () => {
  ...
  const {state, setLimit, setPage, setQuery} = usePagination<MasterComponent, any>(
    useCallback((state, setState, isActive) => {
      const fetchCall = async () => {
        try {
          ...
          const masterComponent = await getMasterComponentList({
            limit: state.limit,
          ...
          setState((prev) => ({
            ...
            error: false,
            page: 1,
            data: masterComponent.data.data ?? [],
            total: 0,
            status,
          }))
          }
  ...
  useEffect(() => {
    document.title = getTitle('List of Master Component')
  }, [])
  ...
  return (
    <>
      <PageTitle>Manage Master Component</PageTitle>
      <div className='card shadow-sm'>
        <div className='card-header d-flex align-items-center justify-content-between'>
          <div className='flex-fill fs-2 fw-bolder'>
            {MasterComponentScreens.LIST_MASTER_COMPONENT.TITLE}
          </div>
          <div className='card-toolbar'>
            <Link
              to={{
                pathname: `/master-component/add`,
              }}
                  ...
                  {state.data.length === 0 ? (
                    ...
                    state.data.map((value, index) => (
                      <tr key={index}>
                        ...
                        <td className='align-middle'>{value.component_id}</td>
                        <td className='align-middle'>{value.component_name}</td>
                        ...
                        <td className='align-middle' style={{minWidth: 125}}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              history.push(`/master-component/edit/${value.component_id}`)
                            }
                          >
        ...
        <DeleteMasterComponentModal
          onDelete={() => {
            deleteMasterComponent(handleDeleteData?.component_id ?? '')
              .then(() => {
