...
import DeleteMasterConditionTypeModal from '../components/DeleteMasterConditionTypeModal'
import {deleteMasterConditionType, getMasterConditionTypeList} from '../redux/MasterConditionTypeCRUD'
import {getErrorMessage} from 'src/app/utils/api-utils'
import Pagination from 'src/app/components/Pagination'

const ListMasterConditionType: FC = () => {
  ...
  const {state, setLimit, setPage, setQuery} = usePagination<MasterConditionType, any>(
    useCallback((state, setState, isActive) => {
      const fetchCall = async () => {
        try {
          ...
          const masterBenefitType = await getMasterConditionTypeList({
            limit: state.limit,
  ...
  useEffect(() => {
    document.title = getTitle('List of Master Condition Type')
  }, [])
  ...
  return (
    <>
      ...
      <div className='card shadow-sm'>
                  ...
                  {state.data.length === 0 ? (
                    ...
                    state.data.map((value, index) => (
                      <tr key={index}>
                        ...
                        <td className='align-middle'>{value.condition_type}</td>
                        ...
                        <td className='align-middle' style={{minWidth: 125}}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              history.push(`/master-type-condition/edit/${value.condition_type_id}`)
                            }
                          >
        ...
        <DeleteMasterConditionTypeModal
          onDelete={() => {
            deleteMasterConditionType(handleDeleteData?.condition_type_id ?? '')
              .then(() => {
...
export {ListMasterConditionType}
