...
import { createMasterRules } from "../redux/MasterRulesCRUD"
...
export const AddMasterRules: FC = (props: any) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values: any, {setStatus, setSubmitting}) => {
      ...
      setTimeout(() => {
        ...
        createMasterRules(values.rule_name ,values.description, values.status)
          .then(() => {
            setLoading(false)
            setSubmitting(false)
            history.goBack()
  ...
  //start::TITLE_FUNC
  const pageTitle = useMemo(() => MasterRulesScreens.ADD_MASTER_RULES.TITLE, [])
