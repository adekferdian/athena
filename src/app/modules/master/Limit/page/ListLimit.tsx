...
import DeleteLimitModal from '../components/DeleteLimitModal'
import {deleteLimit, getLimitList} from '../redux/LimitCRUD'
import {getErrorMessage} from 'src/app/utils/api-utils'
import Pagination from 'src/app/components/Pagination'

const ListLimit: FC = (props: any) => {
  ...
  const {state, setLimit, setPage, setQuery} = usePagination<Limit, any>(
    useCallback((state, setState, isActive) => {
      const fetchCall = async () => {
        try {
          ...
          const masterRules = await getLimitList({
            limit: state.limit,
  ...
  useEffect(() => {
    document.title = getTitle('List of Master Limit')
  }, [])
  ...
  return (
    <>
      <PageTitle>Manage Master Limit</PageTitle>
      <div className='card shadow-sm'>
        <div className='card-header d-flex align-items-center justify-content-between'>
          <div className='flex-fill fs-2 fw-bolder'>
            {LimitScreens.LIST_MASTER_LIMIT.TITLE}
          </div>
          <div className='card-toolbar'>
            <Link
              to={{
                pathname: `/master-limit/add`,
              }}
                  ...
                  {state.data.length === 0 ? (
                    ...
                    state.data.map((value, index) => (
                      <tr key={index}>
                        ...
                        <td className='align-middle'>{value.limit_id}</td>
                        <td className='align-middle'>{value.limit_name}</td>
                        <td className='align-middle'>{value.limit_description}</td>
                        <td className='align-middle'>{value.limit_rules}</td>
                        <td className='align-middle'>{value.limit_type}</td>
                        ...
                        <td className='align-middle' style={{minWidth: 125}}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              history.push(`/master-limit/edit/${value.limit_id}`)
                            }
                          >
        ...
        <DeleteLimitModal
          onDelete={() => {
            deleteLimit(handleDeleteData?.limit_id ?? '')
              .then(() => {
